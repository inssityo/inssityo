<template>
  <div style="width: 100%">
    <select v-if="idValue === 'P'" v-model="age" v-on:click="emitToParent">
      <option value="" selected hidden disabled>{{ageGroup ? optionAges[ageGroup-1].text : "Ikä"}}</option>
      <option
        v-for="(age, index) in optionAges"
        :value="age.value"
        :key="index"
      >
        {{ age.text }}
    ...
    <div v-else class="multiselect">
      ...
      <div id="checkboxes-age" class="checkboxes check__label-only">
        ...
        >
          ...
          <label :for="idValue + 'a' + index" @click="handleAges(age.value)">{{
            age.text
          }}</label>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      optionAges: [
        { text: "alle 20v.", value: 1, checked: false },
        { text: "20-24v.", value: 2, checked: false },
        { text: "25-29v.", value: 3, checked: false },
        { text: "30-34v.", value: 4, checked: false },
        { text: "35-39v.", value: 5, checked: false },
        { text: "40-44v.", value: 6, checked: false },
        { text: "45-49v.", value: 7, checked: false },
        { text: "yli 50v.", value: 8, checked: false },
      ],
      age: "",
      ages: [],
      expanded: false,
